...
with open('Session 4/w2v/vocab-raw.txt', encoding='unicode_escape') as f:
    vocab = dict([(word, word_ID+2) for word_ID, word in enumerate(f.read().splitlines())])
with open('Session 4/w2v/20news-train-encoded.txt', encoding= 'unicode_escape') as f:
...
with open('Session 4/w2v/20news-test-encoded.txt', encoding= 'unicode_escape') as f:
    data = f.read().splitlines()
...
epochs = 50
with open('Session 4/log.txt', 'w') as f:
